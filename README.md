...
- **[landing-zones/](./landing-zones/)** — Complete (7/7). Multi-account AWS Organizations, Azure Management Group hierarchies, and GCP Organization design (all three clouds at deep parity). Account vending, baseline SCPs / Azure Policy / GCP Organization Policy, Control Tower vs Azure Landing Zones vs Cloud Foundation Toolkit, the six-week adoption arc with retrofit patterns, and the five recurring landing-zone failure patterns with corrective moves.
- **[identity-and-access/](./identity-and-access/)** — Complete (7/7). Workforce identity (Entra ID, Okta, AWS Identity Center) and workload identity (IAM Roles for Service Accounts, GCP Workload Identity Federation, OIDC federation from GitHub / GitLab / CI providers). Federation patterns (SAML / OIDC / SCIM with cross-cloud federation). JIT / PIM patterns across all clouds. Service-account lifecycle and inventory discipline. IAM anti-pattern catalogue with tiered remediation order. Least-privilege workflow tied to CloudTrail and IAM Access Analyzer.
- **[network-security/](./network-security/)** — Complete (7/7). VPC / VNet design, hub-and-spoke + Transit Gateway / Cloud WAN, segmentation patterns, egress filtering (Network Firewall, Azure Firewall, GCP Cloud NGFW), PrivateLink / Private Endpoints / Private Service Connect, DNS firewall, and the inspection-architecture decision tree.
- **[data-security/](./data-security/)** — Complete (7/7). KMS strategy and key hierarchy, encryption at rest and in transit, BYOK / HYOK / CMK trade-offs, S3 / Azure Storage / Cloud Storage hardening, RDS / Azure SQL / Cloud SQL security, tokenization, and a DLP pattern that integrates with detection rather than living next to it.
- **[kubernetes-and-container-security/](./kubernetes-and-container-security/)** — Complete (9/9). EKS / AKS / GKE security baselines, Pod Security Standards, network policies, OPA / Gatekeeper / Kyverno, runtime security (Falco, Tetragon), service mesh (Istio, Linkerd), image signing and attestation (Cosign, Sigstore, SLSA), and supply-chain controls from registry to admission.
- **[iac-security/](./iac-security/)** — Complete. Terraform / OpenTofu / Bicep / CloudFormation / Pulumi patterns; module design with secure defaults; policy-as-code (OPA / Conftest, Checkov, Trivy config, Sentinel, CFN Guard, Crossguard); IaC pipeline gates with progressive gating + severity-to-action mapping; drift detection across tools; intentionally-misconfigured reference bundles for scanner validation.
- **[secrets-and-keys/](./secrets-and-keys/)** — Complete (7/7). Secrets Manager, Key Vault, GCP Secret Manager, dynamic credentials with Vault, rotation patterns, KMS key policies, envelope encryption, secret detection, and the OIDC-federation-replaces-long-lived-keys playbook.
- **[cloud-detection-response/](./cloud-detection-response/)** — Complete (10/10). CloudTrail / Activity Logs / Audit Logs architecture; GuardDuty / Defender for Cloud / Security Command Center per-plan selection and tuning; SIEM integration patterns (Splunk, Sentinel, Chronicle, Elastic, Datadog) with the log-router pattern; 25-rule custom detection catalogue across 8 categories; hypothesis-driven threat hunting; five core IR runbooks (leaked-IAM-key, exposed-storage, EKS-pod-compromise, account-takeover, cryptomining).
- **[zero-trust-cloud/](./zero-trust-cloud/)** — Complete (7/7). Identity-aware proxies (Cloudflare Access, AWS Verified Access, GCP IAP), microsegmentation, service-to-service authn with SPIFFE / SPIRE, mTLS service mesh, continuous authorization, and the BeyondCorp-on-existing-stack adoption path that does not require ripping out the network.
- **[serverless-and-paas-security/](./serverless-and-paas-security/)** — Complete (7/7). Lambda / Azure Functions / Cloud Functions and Cloud Run security, API Gateway / API Management / Apigee, App Service + Beanstalk + App Engine + Cloud Run, edge functions (CloudFront Functions, Cloudflare Workers), serverless detection and IAM patterns.
- **[threat-modeling-cloud/](./threat-modeling-cloud/)** — Complete (6/6). Cloud-specific STRIDE templates, MITRE ATT&CK for Cloud Matrix coverage, attack-path analysis, the facilitation guide for cloud threat-modeling sessions, and two worked threat models against multi-account SaaS and multi-cloud data platform.
- **[compliance-and-control-mapping/](./compliance-and-control-mapping/)** — Complete (8/8). HIPAA Security Rule, SOC 2 Trust Services Criteria, PCI-DSS v4, FedRAMP Moderate / High, and NIST 800-53 Rev 5 mapped row-by-row to concrete cloud controls. Continuous controls monitoring architecture; cloud-provider shared-responsibility / Customer Responsibility Matrix; evidence collection runbook with chain-of-custody discipline.
- **incident-response-cloud/** — Folded into [cloud-detection-response/](./cloud-detection-response/) rather than split out, because the same logging architecture has to serve both detection and IR.
