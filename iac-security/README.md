...
- **terraform-security-patterns.md** *(coming)* — Terraform and OpenTofu patterns: state-file protection (S3 + DynamoDB + KMS encryption, Azure Storage + state-lock, GCS + KMS), workspace separation, sensitive variable handling, the `for_each` versus `count` decision for idempotency, the module-versioning discipline that makes upgrades safe.
...
- **iac-pipeline-gates.md** *(coming)* — A reference CI pipeline that runs `terraform plan`, runs Checkov / tfsec / Trivy config / Conftest against the plan, comments findings on the PR, fails the merge on policy violations, and ships SARIF to the GitHub Security tab. With an opinionated severity-to-action mapping.
