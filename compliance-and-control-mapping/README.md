...
- **[continuous-controls-monitoring.md](./continuous-controls-monitoring.md)** — CCM architecture; per-source instrumentation (CSPM, SIEM, IdP, cloud-native, EDR, vuln scanners); per-control evidence pipeline; compliance platform vs build vs hybrid; per-framework tactical guidance. Worked Meridian 12-month rollout with 18 CCM findings.
- **[csp-shared-responsibility.md](./csp-shared-responsibility.md)** — Shared-responsibility model translated into concrete control inheritance; per-cloud attestation portals (AWS Artifact, Azure Service Trust Portal, GCP Compliance Reports Manager); Customer Responsibility Matrix; common misunderstandings. Worked Meridian CRM construction with 18 CRM findings.
- **[evidence-collection-runbook.md](./evidence-collection-runbook.md)** — Per-evidence-type collection patterns (configuration, audit log, access review, vuln scan, training, IR, network, encryption); chain-of-custody discipline; the audit package structure; the screenshot-as-evidence anti-pattern; per-framework evidence checklists. Worked Meridian SOC 2 package with 18 EC findings.
